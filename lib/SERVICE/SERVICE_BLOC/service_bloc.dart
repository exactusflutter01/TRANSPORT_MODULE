...
import 'package:trans_module/SERVICE/service_repository.dart';
...
class ServiceBloc extends Bloc<ServiceEvent, ServiceState> {
  ...
  ServiceBloc(this.repository) : super(ServiceState.initial()) {
    on<SearchDialogueDataFetch>(_searchDialogueDataFetch);
    on<Save>(_save);
  }

  _searchDialogueDataFetch(
      SearchDialogueDataFetch event, Emitter<ServiceState> emit) async {
  ...
  _save(Save event, Emitter<ServiceState> emit) async {
    var docNo;
    emit(state.copyWith(isLoading: true));
    try {
